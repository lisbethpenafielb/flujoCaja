...
function accountRow(account: BankAccount): HTMLElement {
  ...
  return h('div', { class: 'flex items-center gap-3 py-2.5 px-1' }, [
    h('div', { class: 'flex-1 min-w-0' }, [
      h('p', { class: 'text-sm font-medium truncate' }, [account.name]),
...
export function renderBankPanel(accounts: BankAccount[]): HTMLElement {
  ...
  return h('div', { class: 'card p-5 flex flex-col gap-1' }, [
    h('div', { class: 'flex items-center justify-between mb-2' }, [
      h('div', {}, [
        h('h3', { class: 'font-semibold', style: 'font-size:15px' }, ['Saldos Bancarios']),
      ...
      h('div', { class: 'text-right' }, [
        ...
        h('p', { class: 'font-semibold tabular-nums', style: 'font-size:18px' }, [formatMoney(total)]),
      ]),
    ]),
    pending > 0
